...
// launch, not just per (moniker, parent_pid) pair. Two concurrent launches
// of the same emulator by the same user share both moniker and parent_pid,
// so without this suffix they would build the identical event name and race
// on CreateEventW.
...
EventResult SandboxEvent::create() {
    ...
    // other's Python watcher too. The per-launch PID suffix above should
    // make this practically unreachable, but treat it as fatal rather than
    // silently sharing the object, mirroring job_objects.py's
    // ERROR_ALREADY_EXISTS handling for the equivalent job-naming case.
